...
## Cliente Http con Spring Cloud Feign en course-service
...
A continuación vemos que el usuario con `id=3` se ha des-asignado correctamente del curso al que estaba asociado.

````bash
$  curl -v -X DELETE http://localhost:8002/api/v1/courses/users/3 | jq
>
< HTTP/1.1 204
````

Para comprobar lo mencionado anteriormente, vamos a solicitar todos los cursos y sus usuarios asociados. Observamos que
únicamente el curso `Java 21` tiene asignado un usuario, los demás cursos están sin usuarios asociados.
...
**Nota**
> Anteriormente, ya había des-asociado algunos usuarios, es por eso que vemos cursos si usuarios.

## Cliente Http con Spring Cloud Feign en user-service

Nuestro microservicio `user-service` también tiene la dependencia de `Open Feign Client`, así que ahora lo vamos a
configurar para poder usarlo en la realización de peticiones al `course-service`.

Iniciamos configurando la clase principal con la anotación `@EnableFeignClients`.
...
Ahora procedemos a crear nuestra interfaz `CourseFeignClient` que será nuestro `FeignClient`.
...
Como estamos trabajando con `Feign Client` necesitamos manejar los errores que se puedan producir, así que para eso,
crearemos un método handler para la excepción `FeignException` tal como se ve a continuación.
...
## Elimina usuario desde el user-service
...
Para verificar que esté funcionando correctamente, vamos a realizar una previa verificación de qué usuarios tiene
asociado el curso con `id = 2`.
...
Observamos en el resultado anterior que tenemos asociados los usuarios con `id=7 y 9`. Ahora, realicemos la
eliminación del usuario con `id = 9` desde el microservicio `user-service`.

````bash
$ curl -v -X DELETE http://localhost:8001/api/v1/users/7 | jq
>
< HTTP/1.1 204
````

Genial, la eliminación se realizó exitosamente, ahora volvemos a realizar la petición inicial para ver que
efectivamente, el usuario ya no está presente.
