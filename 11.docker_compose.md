...
- Si no existe, `Docker Compose` la construirá a partir del `Dockerfile` ubicado en la ruta definida por
  `build.context`.
- Si hicimos cambios en el código fuente y necesitamos reconstruir la imagen, podemos `docker compose up -d --build`
...
levantarlos usando el comando `docker compose up -d --build`.
...
Vamos a realizar una llamada al microservicio `user-service`.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Sat, 14 Jun 2025 05:06:29 GMT
<
{
  "id": 1,
  "name": "Lesly",
...
Ahora, observamos la parte inicial y la última parte del log de nuestro microservicio `user-service` que está dentro del
contenedor `c-user-service`.
...
2025-06-14T04:54:14.599Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Starting UserServiceApplication v0.0.1-SNAPSHOT using Java 21.0.7 with PID 1 (/app/BOOT-INF/classes started by root in /app) 2025-06-14T04:54:14.602Z DEBUG 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Running with Spring Boot v3.4.5, Spring v6.2.6
2025-06-14T04:54:14.604Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : The following 1 profile is active: "default"
...
2025-06-14T05:06:29.078Z DEBUG 1 --- [user-service] [nio-8001-exec-2] org.hibernate.SQL                        :
    select
        u1_0.id,
        u1_0.email,
        u1_0.name,
...
A continuación modificamos el código fuente de ese mismo microservicio. Para ser exactos solo agregaré un mensaje por
...
[+] Building 69.3s (44/44) FINISHED                                                                                                                             
 => [s-user-service internal] load build definition from Dockerfile                                                                                             
 => => transferring dockerfile: 901B                                                                                                                            
 => [s-course-service internal] load build definition from Dockerfile                                                                                           
 => => transferring dockerfile: 794B                                                                                                                            
 => [s-course-service internal] load metadata for docker.io/library/eclipse-temurin:21-jre-alpine                                                               
 ...
 => [s-user-service internal] load .dockerignore                                                                                                                
 => => transferring context: 214B                                                                                                                               
 => [s-user-service dependencies 1/9] FROM docker.io/library/eclipse-temurin:21-jdk-alpine@sha256:2f2f553ce09d25e2d2f0f521ab94cd73f70c9b21327a29149c23a2b63b8e29
 ...
 => CACHED [s-user-service builder 2/4] WORKDIR /app                                                                                                            
 => CACHED [s-user-service runner 3/7] RUN mkdir ./logs                                                                                                         
 => [s-user-service dependencies 2/9] WORKDIR /app                                                                                                              
 => CACHED [s-course-service dependencies 3/9] COPY ./mvnw ./                                                                                                   
 => CACHED [s-course-service dependencies 4/9] COPY ./.mvn ./.mvn                                                                                               
 => CACHED [s-course-service dependencies 5/9] COPY ./pom.xml ./                                                                                                
 => CACHED [s-course-service dependencies 6/9] RUN sed -i -e 's/\r$//' ./mvnw                                                                                   
 ...
 => => naming to docker.io/library/course-service:latest                                                                                                        
 => [s-course-service] resolving provenance for metadata file                                                                                                   
 => [s-user-service dependencies 3/9] COPY ./mvnw ./                                                                                                            
 => [s-user-service dependencies 4/9] COPY ./.mvn ./.mvn                                                                                                        
 => [s-user-service dependencies 5/9] COPY ./pom.xml ./                                                                                                         
 => [s-user-service dependencies 6/9] RUN sed -i -e 's/\r$//' ./mvnw                                                                                            
 => [s-user-service dependencies 7/9] RUN ./mvnw dependency:go-offline                                                                                          
 => [s-user-service dependencies 8/9] COPY ./src ./src                                                                                                          
 => [s-user-service dependencies 9/9] RUN ./mvnw clean package -DskipTests                                                                                      
 => [s-user-service builder 3/4] COPY --from=dependencies /app/target/*.jar ./app.jar                                                                           
 => [s-user-service builder 4/4] RUN java -Djarmode=layertools -jar app.jar extract                                                                             
 => CACHED [s-user-service runner 4/7] COPY --from=builder /app/dependencies ./                                                                                 
 => CACHED [s-user-service runner 5/7] COPY --from=builder /app/spring-boot-loader ./                                                                           
 => CACHED [s-user-service runner 6/7] COPY --from=builder /app/snapshot-dependencies ./                                                                        
 => [s-user-service runner 7/7] COPY --from=builder /app/application ./                                                                                         
 => [s-user-service] exporting to image                                                                                                                         
 => => exporting layers                                                                                                                                         
...
Verificamos que las imágenes se hayan creado correctamente.

````bash
$ docker image ls
REPOSITORY       TAG             IMAGE ID       CREATED         SIZE
user-service     latest          6c583be9dae5   2 minutes ago   271MB
course-service   latest          c50898fb362d   7 days ago      270MB
postgres         17-alpine       f40315d0e8a6   7 days ago      279MB
<none>           <none>          bf5ad1961409   8 days ago      271MB
mysql            8.0.41-debian   4340b8ad7a7c   4 months ago    610MB
````

⚠️ Notar que tenemos el nombre de una imagen y etiqueta así `<none>`. Esto corresponde a la antigua imagen que teníamos
del `user-service`. Además, solo tenemos una imagen con `<none>` porque el único microservicio que modificamos fue
`user-service`.

A continuación verificamos que los contenedores se hayan creado correctamente.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                   COMMAND                  CREATED         STATUS                   PORTS                               NAMES
5a66282d21e6   user-service:latest     "/__cacert_entrypoin…"   8 minutes ago   Up 7 minutes             0.0.0.0:8001->8001/tcp              c-user-service
04a0921a9873   course-service:latest   "/__cacert_entrypoin…"   8 minutes ago   Up 7 minutes             0.0.0.0:8002->8002/tcp              c-course-service
67e483d6fc48   mysql:8.0.41-debian     "docker-entrypoint.s…"   8 minutes ago   Up 7 minutes (healthy)   33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
a738feff9243   postgres:17-alpine      "docker-entrypoint.s…"   8 minutes ago   Up 7 minutes (healthy)   0.0.0.0:5433->5432/tcp              c-postgres
````

Realizamos nuevamente la petición al microservicio de usuarios.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Sat, 14 Jun 2025 05:23:11 GMT
<
{
  "id": 1,
  "name": "Lesly",
...
Ingresamos al log del `c-user-service` y observamos que el mensaje que agregamos está mostrándose perfectamente, eso
significa que nuestra imagen se volvió a construir.
...
2025-06-14T05:14:21.408Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Starting UserServiceApplication v0.0.1-SNAPSHOT using Java 21.0.7 with PID 1 (/app/BOOT-INF/classes started by root in /app) 2025-06-14T05:14:21.411Z DEBUG 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Running with Spring Boot v3.4.5, Spring v6.2.6
...
2025-06-14T05:14:31.441Z  INFO 1 --- [user-service] [           main] o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat started on port 8001 (http) with context path '/'
2025-06-14T05:14:31.479Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Started UserServiceApplication in 10.956 seconds (process running for 11.948)
2025-06-14T05:23:11.751Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.a.c.c.C.[Tomcat].[localhost].[/]       : Initializing Spring DispatcherServlet 'dispatcherServlet'
2025-06-14T05:23:11.751Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Initializing Servlet 'dispatcherServlet'
2025-06-14T05:23:11.753Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Completed initialization in 2 ms
2025-06-14T05:23:11.799Z  INFO 1 --- [user-service] [nio-8001-exec-1] d.m.user.app.controller.UserController   : ¡Cambio efectuado!
2025-06-14T05:23:11.846Z DEBUG 1 --- [user-service] [nio-8001-exec-1] org.hibernate.SQL                        :
    select
        u1_0.id,
        u1_0.email,
        u1_0.name,
