...
## Comunicación entre aplicación en contenedor y aplicación en máquina local

En este apartado veremos cómo comunicar nuestra aplicación dockerizada (`user-service`) con nuestra aplicación que aún
está en nuestra máquina local sin dockerizar (`course-service`).

### user-service

El microservicio `user-service` tiene definido un cliente feign que se comunica con el `course-service`. Entonces,
necesitamos modificar la url de dicho cliente feign para que, desde el interior del contenedor pueda comunicarse al
exterior, hacia nuestro `course-service`.
...
El microservicio `course-service` también tiene definido un cliente feign que se comunica con el `user-service`. En
este caso, como la comunicación es desde la máquina local hacia dentro del contenedor, no es necesario cambiar la url
de dicho cliente feign, dado que el puerto externo que le definimos al contenedor del `user-service` es el mismo que
está usando el cliente feign (`8001`).
...
El único cambio que hicimos fue en el `user-service`, así que como esa aplicación está dockerizada, necesitamos volver
a dockerizarlo para que contenga los cambios que realizamos. Así que, volvemos a generar el `jar` de la aplicación y
luego su respectiva imagen.
...
- `-t user-service`, con el `-t` le damos un nombre a la imagen (REPOSITORY) cuyo tag por defecto será `latest`. Si
  quisiéramos darle un nombre a la imagen con un tag distinto, sería de la siguiente manera `mi-imagen:v1`. Donde
  `mi-imagen` sería el nombre de la imagen (repository) y `v1` el valor del tag.
...
$ docker image ls
REPOSITORY                               TAG             IMAGE ID       CREATED          SIZE
user-service                             latest          fe8e8586debb   14 seconds ago   401MB
````

Ahora procedemos a levantar un contenedor de la image `user-service` y lo listamos.

````bash
$ docker container run -d -p 8001:8001 user-service
fe6dd168ad3d192745d944e29fc36443a3081d01232bcf257af04329404c617f

$ docker container ls -a
CONTAINER ID   IMAGE            COMMAND                  CREATED          STATUS                      PORTS                    NAMES
fe6dd168ad3d   user-service     "/__cacert_entrypoin…"   14 seconds ago   Up 13 seconds               0.0.0.0:8001->8001/tcp   condescending_herschel
````

Una vez que ya tenemos nuestro contenedor del microservicio `user-service` corriendo, levantamos nuestro
microservicio `course-service` que esté en nuestra máquina local. Lo podemos hacer usando el `IDE IntelliJ IDEA`.

Ahora que ambos están levantados ejecutamos el siguiente comando para ver si hay comunicación entre ellos.

### Desde course-service hacia user-service (máquina local hacia contenedor)
...
### Desde user-service hacia course-service (contenedor hacia máquina local)
...
Si vemos el curso con id 2, veremos de que el usuario con id 4 ya no está porque fue eliminado.
...
De esta manera estamos comprobando que la comunicación de afuera hacia adentro y de adentro hacia afuera se realiza
correctamente.
