...
## Modifica `compose.yml` para obtener imágenes desde `Docker Hub`

En nuestro archivo `compose.yml` los servicios `s-user-service` y `s-course-service` están usando los bloques `build`
para construir sus imágenes a partir de un `Dockerfile`.

Veamos un extracto del servicio `s-user-service` para recordar cómo es que estamos construyendo la imagen.
...
En este apartado, en vez de dejar que `Docker Compose` construya la imagen para cada servicio, lo que haremos será
obtener las imágenes de forma remota desde `Docker Hub`, similar a cómo obtenemos las imágenes de `mysql` y `postgres`.
Así que eliminaremos las imágenes de los microservicios y nos quedaremos únicamente con las imágenes de las bases de
datos.
...
Realizamos las modificaciones a los servicios `s-user-service` y `s-course-service` del archivo `compose.yml` para que
las imágenes las traiga desde `Docker Hub`.
...
> Como ahora traeremos las imágenes desde `Docker Hub` ya no necesitamos usar el bloque `build` por eso lo eliminé.

En `image` especificamos la imagen que vamos a usar, primero lo buscará en nuestra máquina local, al no encontrarlo irá
a `Docker Hub` a bajarlo.

Luego de haber modificado el `compose.yml` ejecutamos los comandos de `Docker Compose` para levantar contenedores y
veremos que en automático se empezarán a descargar las imágenes.

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes (feature/section-12) 
$ docker compose up -d                                                                                      
[+] Running 17/17
 ✔ s-user-service Pulled                                                                                    
   ✔ f18232174bc9 Already exists                                                                            
   ✔ f6cd406c8d97 Already exists                                                                            
   ✔ 74f6a226ed93 Already exists                                                                            
   ✔ e6744199aa66 Already exists                                                                            
   ...
   ✔ d886de221ead Already exists                                                                            
   ✔ 52fa11846936 Already exists                                                                            
 ✔ s-course-service Pulled                                                                                  
   ✔ 860565b8e096 Already exists                                                                            
   ✔ 986436684c4a Already exists                                                                            
   ✔ b4201047aca3 Already exists                                                                            
   ✔ 60bf6a350d96 Already exists                                                                            
[+] Running 4/4                                                                                             
 ✔ Container c-mysql           Healthy                                                                      
 ✔ Container c-postgres        Healthy                                                                      
 ✔ Container c-user-service    Started                                                                      
...
Verificamos que todos los contenedores están levantados.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                             COMMAND                  CREATED         STATUS                        PORTS                               NAMES
316500a40620   magadiflo/course-service:latest   "/__cacert_entrypoin…"   2 minutes ago   Up About a minute             0.0.0.0:8002->8002/tcp              c-course-service
45cb66bf2238   magadiflo/user-service:latest     "/__cacert_entrypoin…"   2 minutes ago   Up About a minute             0.0.0.0:8001->8001/tcp              c-user-service
d4efada9fc41   postgres:17-alpine                "docker-entrypoint.s…"   2 minutes ago   Up About a minute (healthy)   0.0.0.0:5433->5432/tcp              c-postgres
646ebf6a3612   mysql:8.0.41-debian               "docker-entrypoint.s…"   2 minutes ago   Up About a minute (healthy)   33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
````

Realizamos una petición para ver que los 4 contenedores están funcionando bien.
...
< Date: Mon, 16 Jun 2025 00:03:49 GMT
<
{
  "id": 1,
  "name": "Spring Boot 3",
