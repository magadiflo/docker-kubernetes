...
$ docker container ls -a
CONTAINER ID   IMAGE            COMMAND                  CREATED         STATUS                    PORTS                    NAMES
3e0760845176   course-service   "/__cacert_entrypoin…"   9 minutes ago   Up 9 minutes              0.0.0.0:8002->8002/tcp   course-service
1838ae89cf65   user-service     "/__cacert_entrypoin…"   9 minutes ago   Up 9 minutes              0.0.0.0:8001->8001/tcp   user-service
````

Ahora, volvemos a crear los contenedores de las bases de datos.

````bash
$ docker container run -d -p 3307:3306 --name c-mysql --network docker-network -e MYSQL_ROOT_PASSWORD=magadiflo -e MYSQL_DATABASE=db_user_service -e MYSQL_USER=admin -e MYSQL_PASSWORD=magadiflo -v data_mysql:/var/lib/mysql --restart=unless-stopped mysql:8.0.33
630e958cae5b5dfdaf04e9ac2f6825bbc7cb91f44a793b8d4409f11bc26663d8

$  docker container run -d -p 5433:5432 --name c-postgres --network docker-network -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=magadiflo -e POSTGRES_DB=db_course_service -v data_postgres:/var/lib/postgresql/data --restart=unless-stopped postgres:15.2-alpine
23ebfb070dfd49ce1c540c00fa56b67843fd6485631c21fb480a336c516993ef
````
...
$ docker container ls -a
CONTAINER ID   IMAGE                  COMMAND                  CREATED          STATUS                    PORTS                               NAMES
630e958cae5b   mysql:8.0.33           "docker-entrypoint.s…"   38 seconds ago   Up 38 seconds             33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
23ebfb070dfd   postgres:15.2-alpine   "docker-entrypoint.s…"   59 seconds ago   Up 58 seconds             0.0.0.0:5433->5432/tcp              c-postgres
3e0760845176   course-service         "/__cacert_entrypoin…"   11 minutes ago   Up 11 minutes             0.0.0.0:8002->8002/tcp              course-service
1838ae89cf65   user-service           "/__cacert_entrypoin…"   11 minutes ago   Up 11 minutes             0.0.0.0:8001->8001/tcp              user-service
````
...
En este apartado crearemos unos contenedores especiales, digamos que serán `clientes` de las bases de datos `MySQL` y
`PostgreSQL`. Estos contenedores nos permitirá habilitar la línea de comando para interactuar y realizar consultas a
los contenedores de las bases de datos antes mencionados. Los llamaremos `contenedores utilitarios`, dado que no ejecuta
una instancia de `MySQL` o `PostgresSQL` sino que solo nos permite conectarnos a esas bases de datos para interactuar
con ellas a través de una línea de comandos.

En este punto del curso tenemos únicamente los contenedores de bases de datos ejecutándose.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                  COMMAND                  CREATED        STATUS                    PORTS                               NAMES
630e958cae5b   mysql:8.0.33           "docker-entrypoint.s…"   23 hours ago   Up 27 minutes             33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
23ebfb070dfd   postgres:15.2-alpine   "docker-entrypoint.s…"   23 hours ago   Up 27 minutes             0.0.0.0:5433->5432/tcp              c-postgres
````

### Contenedor utilitario para MySQL

La siguiente instrucción nos permite crear un contenedor utilitario que nos proporciona una línea de comandos para
poder interactuar con la base de datos de `MySQL` que actualmente ya lo tenemos ejecutando en un contenedor.

````bash
$ docker container run -it --rm --network docker-network mysql:8.0.33 /bin/sh
...
- `--network docker-network`, debe ser la misma red en la que se encuentra el contenedor de `MySQL`.
- `mysql:8.0.33`, imagen a partir de la cual crearemos un contenedor al vuelo, un utilitario.
- `/bin/bash` o `bash`, línea de comando que usaremos dentro del contenedor.

Al ingresar a la línea de comando vemos que escribí lo siguiente:

- `mysql -hc-mysql -uadmin -p`, donde:
    - `mysql`, este es el comando del cliente de `MySQL`. Es la herramienta utilizada para interactuar con el servidor
      `MySQL` desde la línea de comandos, permitiéndote ejecutar consultas y gestionar la base de datos.
    - `-hc-mysql`, el host (`-h`) al que nos queremos conectar es el nombre del contenedor (`c-mysql`) donde está
      ejecutándose nuestra base de datos de mysql.
    - `-uadmin`, el usuario (`-u`) es el que definimos al crear el contenedor de mysql, en nuestro caso es `admin`.
    - `-p`, el password es `magadiflo`, será ingresado en la línea siguiente.

### Contenedor utilitario para PostgreSQL

La siguiente instrucción nos permite crear un contenedor utilitario que nos proporciona una línea de comandos para
poder interactuar con la base de datos de `PostgreSQL` que actualmente ya lo tenemos ejecutando en un contenedor.

````bash
$  docker container run -it --rm --network docker-network postgres:15.2-alpine /bin/sh

/ # psql -hc-postgres -Upostgres
...
-------------------+----------+----------+------------+------------+------------+-----------------+-----------------------
 db_course_service | postgres | UTF8     | en_US.utf8 | en_US.utf8 |            | libc            |
...
postgres=# \c db_course_service
You are now connected to database "db_course_service" as user "postgres".
...
- `-it`, habilitar el terminal interactivo.
- `--rm`, cuando se detenga este contenedor, se eliminará automáticamente.
- `--network docker-network`, debe ser la misma red en la que se encuentra el contenedor de `PostgreSQL`.
- `postgres:15.2-alpine`, imagen a partir de la cual crearemos un contenedor al vuelo, un utilitario.
- `/bin/bash` o `bash`, línea de comando que usaremos dentro del contenedor.

Al ingresar a la línea de comando vemos que escribí lo siguiente:

- `psql -hc-postgres -Upostgres`, donde:
    - `psql`, este es el cliente de `PostgreSQL`. Es una herramienta interactiva que te permite conectarte a una base de
      datos `PostgreSQL`, ejecutar consultas `SQL`, y administrar la base de datos desde la línea de comandos.
    - `-hc-postgres`, el host (`-h`) al que nos queremos conectar es el nombre del contenedor (`c-postgres`) donde está
      ejecutándose nuestra base de datos de postgresql.
    - `-Upostgres`, el usuario (`-U`) es el que definimos al crear el contenedor de postgres, en nuestro caso es
      `postgres`.
    - Luego, cuando demos click en esas instrucciones, en la siguiente línea pedirá ingresar el password que es
      `magadiflo`.
