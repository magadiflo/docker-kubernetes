...
El volumen `hostPath` permite montar un archivo o directorio del sistema de archivos del nodo (Worker Node)
directamente dentro del Pod.
...
- `PersistentVolume` no es un tipo de volumen físico en sí.
- Es un objeto de Kubernetes que actúa como un wrapper o abstracción de más alto nivel que se respalda internamente en
  uno de los tipos reales de almacenamiento, como por ejemplo:
    - `hostPath` (almacenamiento local en el nodo)
    - `nfs` (almacenamiento en red)
    - `awsElasticBlockStore`, `gcePersistentDisk` (almacenamiento en la nube)
    - `csi` (provisión mediante drivers de terceros)
- Se configura en un archivo `YAML` independiente, `fuera del Deployment`, lo que permite su reutilización por múltiples
  Pods.
...
## 🔗 Modificando el Deployment de MySQL para usar el PersistentVolumeClaim (PVC)

Hasta este punto, ya definimos el `PersistentVolume (PV)` y su respectivo `PersistentVolumeClaim (PVC)`.
El siguiente paso es conectar ese `PVC` al contenedor de `MySQL` dentro del archivo `deployment-mysql.yml`.

Esto permitirá que el contenedor monte el volumen de forma persistente, utilizando el `claim` previamente creado.

### 📄 Archivo actualizado: `deployment-mysql.yml`
...
- El `PVC` actúa como intermediario para acceder al `PersistentVolume (pv-mysql)`, el cual está respaldado por un
...
> ✅ `Recordatorio`: El `PVC` actúa como intermediario entre el `Pod` y el `PV`.
> Es a través de él que se establece el vínculo entre el contenedor y el volumen físico gestionado por `Kubernetes`.

## Creando nuevos objetos: Persistent Volumes (PV) y Persistent Volume Claims (PVC)

> `Recordemos`: El `PersistentVolume (PV)` es un volumen persistente que está gestionado por el clúster, externo al pod,
> y puede ser accedido por cualquier `pod` a través de un `PersistentVolumeClaim (PVC)`.

Aplicamos el `Persistent Volumes (PV)` del archivo `pv-mysql.yml`.

````bash
$ kubectl apply -f .\kubernetes\volumes\pv-mysql.yml
persistentvolume/pv-mysql created
````

Aplicamos el `Persistent Volume Claims (PVC)` del archivo `pvc-mysql.yml`.

````bash
$ kubectl apply -f .\kubernetes\volumes\pvc-mysql.yml
persistentvolumeclaim/pvc-mysql created
````

Listamos el `Persistent Volumes (PV)` creado.

````bash
$ kubectl get pv
NAME       CAPACITY   ACCESS MODES   RECLAIM POLICY   STATUS   CLAIM               STORAGECLASS   VOLUMEATTRIBUTESCLASS   REASON   AGE
pv-mysql   2Gi        RWO            Retain           Bound    default/pvc-mysql   standard       <unset>                          91s
````

> `RWO`, siglas de `ReadWriteOnce`

Listamos el `Persistent Volume Claims (PVC)` creado.

````bash
$ kubectl get pvc
NAME        STATUS   VOLUME     CAPACITY   ACCESS MODES   STORAGECLASS   VOLUMEATTRIBUTESCLASS   AGE
pvc-mysql   Bound    pv-mysql   2Gi        RWO            standard       <unset>                 88s
````

## 📦 Aplicando los cambios en el Deployment de MySQL

Antes de aplicar los cambios en el archivo `deployment-mysql.yml` —donde ahora se utiliza un
`PersistentVolumeClaim (PVC)`—, revisamos el estado actual de los deployments y los pods:
...
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS      AGE
d-course-service-5f7bdcf6dc-hs8z5   1/1     Running   3 (38m ago)   4d23h
d-mysql-7c844dccd9-xgkn8            1/1     Running   3 (39m ago)   5d23h
d-postgres-5984c94d6f-7q62p         1/1     Running   2 (39m ago)   4d23h
d-user-service-7bc4ffb6df-l8c5l     1/1     Running   4 (38m ago)   5d23h 
````

### 🚀 Aplicando el nuevo deployment

Una vez creados los objetos `PersistentVolume (PV)` y `PersistentVolumeClaim (PVC)`, procedemos a aplicar el deployment
de `MySQL` con soporte para almacenamiento persistente:
...
Volvemos a listar los deployments y pods para confirmar que se ha creado un nuevo pod asociado al deployment
actualizado:
...
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS      AGE
d-course-service-5f7bdcf6dc-hs8z5   1/1     Running   3 (39m ago)   4d23h
d-mysql-5dcd7bc77f-v8cch            1/1     Running   0             20s
d-postgres-5984c94d6f-7q62p         1/1     Running   2 (40m ago)   4d23h
d-user-service-7bc4ffb6df-l8c5l     1/1     Running   4 (39m ago)   5d23h
````

> 🔄 Como se observa, el antiguo pod de MySQL fue reemplazado por uno nuevo que incorpora el PVC.

Si inspeccionamos el nuevo pod con `kubectl describe`, podremos confirmar que está usando el `PersistentVolumeClaim`
definido:

````bash
kubectl describe pod d-mysql-5dcd7bc77f-v8cch
Name:             d-mysql-5dcd7bc77f-v8cch
Namespace:        default
Priority:         0
Service Account:  default
Node:             minikube/192.168.49.2
Start Time:       Wed, 30 Jul 2025 23:49:48 -0500
Labels:           app=d-mysql
                  pod-template-hash=5dcd7bc77f
Annotations:      <none>
Status:           Running
IP:               10.244.0.78
IPs:
  IP:           10.244.0.78
Controlled By:  ReplicaSet/d-mysql-5dcd7bc77f
Containers:
  c-mysql:
    Container ID:   docker://3f59ef2966b4b0177de51a0f299bd15452aed29c09cbfaf7e08cd40eb90ae1cd
    Image:          mysql:8.0.41-debian
    Image ID:       docker-pullable://mysql@sha256:b2252987e0ecdb820e96928948ac3bca1adcd2b4a2a2c7b0d7ea78f77a9dc6ac
    Port:           3306/TCP
    Host Port:      0/TCP
    State:          Running
      Started:      Wed, 30 Jul 2025 23:49:49 -0500
    Ready:          True
    Restart Count:  0
    Environment:
      MYSQL_ROOT_PASSWORD:  magadiflo
    ...
    Mounts:
      /var/lib/mysql from mysql-data (rw)
      /var/run/secrets/kubernetes.io/serviceaccount from kube-api-access-5kz9p (ro)
Conditions:
  Type                        Status
  PodReadyToStartContainers   True
  Initialized                 True
...
Events:
  Type    Reason     Age    From               Message
  ----    ------     ----   ----               -------
  Normal  Scheduled  3m58s  default-scheduler  Successfully assigned default/d-mysql-5dcd7bc77f-v8cch to minikube
  Normal  Pulled     3m57s  kubelet            Container image "mysql:8.0.41-debian" already present on machine
  Normal  Created    3m57s  kubelet            Created container: c-mysql
  Normal  Started    3m57s  kubelet            Started container c-mysql
````

> ✅ Con esto, el pod de MySQL ya está usando almacenamiento persistente gestionado por Kubernetes a través del `PVC`
> `pvc-mysql`.

## ✅ Probando el funcionamiento el microservicio usuarios con `PersistentVolume`

Para validar que el microservicio de usuarios sigue funcionando correctamente con su nueva configuración de
almacenamiento persistente (basada en `PersistentVolume`), primero obtenemos la URL del servicio:
...
### 📌 Verificando persistencia de los datos

Pese a haber reemplazado el tipo de volumen (de `hostPath directo` a `PersistentVolume`), los datos siguen estando
disponibles:

````bash
$ curl -v http://127.0.0.1:60013/api/v1/users | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Thu, 31 Jul 2025 05:13:59 GMT
<
[
  {
    "id": 1,
...
### 🧠 ¿Por qué se conservaron los datos?

Esto ocurre porque:

- En el contenedor de MySQL estamos utilizando como `mountPath` la ruta `/var/lib/mysql`, que es la ubicación interna
  por defecto donde la imagen oficial de MySQL almacena sus datos.
- Esta ruta está montada a un volumen que ahora apunta a un `PersistentVolume (PV)` configurado con:
  ````yml
  hostPath:
    path: /var/lib/mysql
  ````
- Dado que el `PV` sigue apuntando al mismo directorio físico en el nodo (`/var/lib/mysql`), los datos antiguos
  permanecen disponibles aunque el pod haya sido destruido y recreado.

## 🔄 ¿Qué cambió respecto al volumen?

Antes, en el archivo `deployment-mysql.yml`, el volumen se definía directamente dentro del `pod` con `hostPath`:
...
Esto hacía que el `pod` tuviera acceso directo a una ruta local del nodo (`hostPath`). Aunque los datos persistían,
esta solución estaba acoplada al nodo específico y no era ideal para entornos distribuidos.

Ahora, ese mismo `hostPath` lo trasladamos a un recurso independiente del `pod` llamado `PersistentVolume (PV)`:
...
Y el pod de MySQL accede al almacenamiento a través de un `PersistentVolumeClaim (PVC)`, lo que desacopla la
configuración y permite una gestión más flexible del almacenamiento.

### ✅ ¿Qué logramos con esto?

- Abstraer el volumen como un recurso del clúster `(PV)`, no dependiente del pod.
- Mantener la persistencia de datos, ya que `hostPath` sigue apuntando al mismo lugar.
- Mejorar la reutilización, porque ahora cualquier pod con el `PVC` adecuado puede acceder a este almacenamiento.
- Dar un paso hacia buenas prácticas, ya que evitamos definir almacenamiento directamente dentro del deployment del pod.
- En resumen, no cambiamos la ruta del almacenamiento (`/var/lib/mysql`), pero sí cambiamos la forma en que Kubernetes
  lo gestiona, haciéndolo más limpio, modular y reutilizable.

## 🧱 Configurando un PersistentVolume (PV) para PostgreSQL

Anteriormente, habíamos visto paso a paso cómo crear la configuración del `PersistentVolume (PV)` y del
`PersistentVolumeClaim (PVC)` para mysql. En esta lección veremos cómo realizar lo mismo, pero para nuestra base de
datos de Postgres. Por lo tanto, no me enfocaré tanto en explicar detalles, simplemente veremos los pasos a realizar.

Primer paso, creamos el archivo para nuestro `PersistentVolume (PV)` para postgres llamado `pv-postgres.yml`.
...
Segundo paso, creamos el archivo para el `PersistentVolumeClaim (PVC)` llamado `pvc-postgres.yml`.
...
Tercer paso, modificamos el `deployment-postgres.yml` para que ahora utilice el `PersistentVolume (PV)` a través del
`PersistentVolumeClaim (PVC)`.
...
## 🧱 Aplicando PersistentVolume (PV), PersistentVolumeClaim (PVC) y deployment para PostgreSQL

En esta lección aplicaremos los dos archivos persistent que creamos anteriormente y la modificación realizada al
`deployment-postgres.yml`.

````bash
$ kubectl apply -f .\kubernetes\volumes\pv-postgres.yml -f .\kubernetes\volumes\pvc-postgres.yml -f .\kubernetes\deployments\deployment-postgres.yml
persistentvolume/pv-postgres created
persistentvolumeclaim/pvc-postgres created
deployment.apps/d-postgres configured
````

Verificamos la creación de los persistents

````bash
$ kubectl get pv
NAME          CAPACITY   ACCESS MODES   RECLAIM POLICY   STATUS   CLAIM                  STORAGECLASS   VOLUMEATTRIBUTESCLASS   REASON   AGE
pv-mysql      2Gi        RWO            Retain           Bound    default/pvc-mysql      standard       <unset>                          23h
pv-postgres   2Gi        RWO            Retain           Bound    default/pvc-postgres   standard       <unset>                          67s
````

````bash
$ kubectl get pvc
NAME           STATUS   VOLUME        CAPACITY   ACCESS MODES   STORAGECLASS   VOLUMEATTRIBUTESCLASS   AGE
pvc-mysql      Bound    pv-mysql      2Gi        RWO            standard       <unset>                 23h
pvc-postgres   Bound    pv-postgres   2Gi        RWO            standard       <unset>                 79s
````

## ✅ Probando el funcionamiento el microservicio cursos con `PersistentVolume`

Obtenemos la url del servicio `s-course-service`.

````bash
$ minikube service s-course-service --url
http://127.0.0.1:51279
! Because you are using a Docker driver on windows, the terminal needs to be open to run it.
````

Ahora, realizamos una petición al endpoint del microservicio de cursos para obtener el detalle de un curso específico,
incluso le solicitamos que nos traiga los usuarios relacionados, de esta manera comprobamos que la comunicación entre
el microservicio de cursos y usuarios se está efectuando correctamente.

````bash
$ curl -v -G --data "loadRelations=true" http://localhost:51279/api/v1/courses/1 | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 01 Aug 2025 04:39:41 GMT
<
{
  "id": 1,
  "name": "Spring Boot 3",
