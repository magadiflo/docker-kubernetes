...
Si revisamos el archivo `.env` del `user-service`, veremos que originalmente teníamos una variable de entorno.
...
Este valor (`s-mysql`) lo utilizábamos cuando trabajábamos con `compose.yml`. Ahora que estamos usando `Kubernetes` y
hemos creado el servicio con el mismo nombre (`s-mysql`), podemos mantener esta configuración sin necesidad de
cambiarla.

En caso de que el nombre del servicio fuera diferente, deberíamos ajustar el valor de la variable `DB_HOST` para que
coincida con el nombre del `Service` que expone `MySQL`.
