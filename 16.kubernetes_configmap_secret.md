...
Recordemos que los objetos `ConfigMap` no están diseñados para almacenar datos confidenciales, ya que no aplican ningún
mecanismo de encriptación o restricción de acceso por defecto. Si los datos que deseamos almacenar son sensibles, como
contraseñas o credenciales, debemos utilizar un objeto `Secret`.

Un `Secret` es un recurso de `Kubernetes` diseñado específicamente para contener pequeñas cantidades de datos sensibles,
como contraseñas, tokens o claves de acceso. Al usar `Secrets`, evitamos incluir datos privados directamente en el
código fuente o en los manifiestos de despliegue.

### Objetivo de esta lección

Hasta este punto, hemos almacenado temporalmente las credenciales de conexión a las bases de datos dentro de los
`ConfigMaps`. Sin embargo, como se trata de información sensible, a partir de ahora las moveremos a archivos `Secret`,
...
- `type: Opaque` indica que se trata de un secreto genérico definido por el usuario.
- Los valores de las claves (`db_username`, `db_password`) deben estar codificados en `Base64`, no en texto plano. Por
  ejemplo:
    - `admin` → `YWRtaW4=`
    - `magadiflo` → `bWFnYWRpZmxv`
- Podemos usar herramientas en línea como [base64encode](https://www.base64encode.org/) o ejecutar el siguiente comando
...
> como una opción. En ese caso, se recomienda utilizar `Git Bash`, `WSL` o una terminal compatible.

⚠️ Importante 2
> Ahora que hemos creado archivos `Secret` para almacenar las credenciales de conexión, debemos eliminar esos datos
...
`Deployments`. La forma de hacerlo es a través de la propiedad `env.valueFrom.secretKeyRef`, que permite inyectar
valores directamente desde un `Secret` como variables de entorno.

A continuación, mostraremos cómo se configura cada uno de los archivos `Deployment`.

`deployment-user.yml`. Configuramos el contenedor del microservicio `user-service` para que obtenga las credenciales de
conexión desde el `Secret` llamado `sec-user-service`:
...
`deployment-mysql.yml`. El contenedor del servicio de base de datos `MySQL` también necesita acceder a credenciales.
Aquí se reutiliza el `Secret` `sec-user-service` para inyectar las variables correspondientes al entorno de `MySQL`:
...
`deployment-postgres.yml`. Por último, configuramos el contenedor de la base de datos `PostgreSQL` para que utilice el
`Secret` `sec-course-service`:
...
> ⚠️ Asegúrate de que los nombres de los `Secrets` y las claves (`db_username`, `db_password`, etc.) coincidan
> exactamente con los definidos en tus archivos `Secret`. `Kubernetes` no hace validación automática y fallará
> silenciosamente si hay errores de nombre.

## Aplicar secretos configurados en los Deployments

Una vez que tenemos los archivos `Secret` creados y referenciados en los `Deployments`, procedemos a aplicarlos al
clúster.

1. Aplicar los archivos `Secret`

````bash
$ kubectl apply -f .\kubernetes\secrets\secret-user.yml -f .\kubernetes\secrets\secret-course.yml
secret/sec-user-service created
secret/sec-course-service created
````

2. Verificar que los `Secrets` fueron creados

````bash
$ kubectl get secrets
NAME                 TYPE     DATA   AGE
sec-course-service   Opaque   2      17s
sec-user-service     Opaque   2      17s
````

3. Aplicar nuevamente los `Deployments`. Esto es necesario porque ahora los contenedores hacen uso de variables de
   entorno basadas en `Secrets`.
...
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS       AGE
d-course-service-6c87fd54bc-hw4ds   1/1     Running   0              4m27s
d-mysql-5ffd47dd8-ctmt8             1/1     Running   0              4m28s
d-postgres-67b99b466d-c27wb         1/1     Running   1 (2m5s ago)   4m27s
d-user-service-8655c4b7d8-6h9hr     1/1     Running   0              4m27s
````

5. Describir un `Pod` y validar variables de entorno. Con `kubectl describe pod` podemos verificar que las variables de
   entorno están siendo tomadas desde los `ConfigMaps` y `Secrets` correctamente.

````bash
$ kubectl describe pod d-user-service-8655c4b7d8-6h9hr
Name:             d-user-service-8655c4b7d8-6h9hr
Namespace:        default
Priority:         0
Service Account:  default
Node:             minikube/192.168.49.2
Start Time:       Mon, 04 Aug 2025 00:54:59 -0500
Labels:           app=d-user-service
                  pod-template-hash=8655c4b7d8
Annotations:      <none>
Status:           Running
IP:               10.244.0.106
IPs:
  IP:           10.244.0.106
Controlled By:  ReplicaSet/d-user-service-8655c4b7d8
Containers:
  c-user-service:
    Container ID:   docker://9705188a7337147d74ea73f2dc5fb3bbe8a5da910aa93b7887edc1eda1367dc1
    Image:          magadiflo/user-service:latest
    Image ID:       docker-pullable://magadiflo/user-service@sha256:d0edef392f95e3e3e082745d166f3f86da4fe3d29d86a3300ce0bcfb5ef68a4b
    Port:           8001/TCP
    Host Port:      0/TCP
    State:          Running
      Started:      Mon, 04 Aug 2025 00:56:12 -0500
    Ready:          True
    Restart Count:  0
    Environment:
      CONTAINER_PORT:       <set to the key 'container_port' of config map 'cm-user-service'>       Optional: false
      ...
      COURSE_SERVICE_PORT:  <set to the key 'course_service_port' of config map 'cm-user-service'>  Optional: false
    Mounts:
      /var/run/secrets/kubernetes.io/serviceaccount from kube-api-access-vxghd (ro)
Conditions:
  Type                        Status
  PodReadyToStartContainers   True
  Initialized                 True
...
Events:
  Type     Reason     Age                    From               Message
  ----     ------     ----                   ----               -------
  Normal   Scheduled  5m41s                  default-scheduler  Successfully assigned default/d-user-service-8655c4b7d8-6h9hr to minikube
  Normal   Pulled     5m40s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 1.089s (1.089s including waiting). Image size: 270793294 bytes.
  Normal   Pulled     5m38s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 991ms (1.924s including waiting). Image size: 270793294 bytes.
  Normal   Pulled     5m22s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 1.007s (1.007s including waiting). Image size: 270793294 bytes.
  Normal   Pulled     5m8s                   kubelet            Successfully pulled image "magadiflo/user-service:latest" in 978ms (1.016s including waiting). Image size: 270793294 bytes.
  Normal   Pulled     4m55s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 977ms (977ms including waiting). Image size: 270793294 bytes.
  Warning  Failed     4m43s (x6 over 5m40s)  kubelet            Error: secret "sec-user-service" not found
  Normal   Pulled     4m43s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 971ms (971ms including waiting). Image size: 270793294 bytes.
  Normal   Pulling    4m29s (x7 over 5m41s)  kubelet            Pulling image "magadiflo/user-service:latest"
  Normal   Pulled     4m28s                  kubelet            Successfully pulled image "magadiflo/user-service:latest" in 1.019s (1.019s including waiting). Image size: 270793294 bytes.
  Normal   Created    4m28s                  kubelet            Created container: c-user-service
  Normal   Started    4m27s                  kubelet            Started container c-user-service
````

Con esto hemos verificado que las credenciales sensibles ahora están siendo gestionadas de forma segura mediante
`Secrets`, mientras que el resto de la configuración no sensible sigue manejándose desde `ConfigMaps`.

### Verificar que la aplicación sigue funcionando

Finalmente, probamos que los servicios estén operativos tras aplicar los cambios de configuración con `ConfigMaps` y
`Secrets`.

1. Obtenemos la URL pública del servicio `s-course-service`:
...
2. Realizamos una petición curl al endpoint de cursos para validar la integración entre servicios:

````bash
$ curl -v -G --data "loadRelations=true" http://127.0.0.1:51772/api/v1/courses/1 | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 04 Aug 2025 06:13:51 GMT
<
{
  "id": 1,
  "name": "Spring Boot 3",
...
✅ La respuesta 200 OK indica que todo funciona correctamente y que las configuraciones desde los `Secrets` y
`ConfigMaps` han sido aplicadas exitosamente en los contenedores.
