...
El microservicio `user-service` utiliza el `RestClient` para comunicarse con el `course-service`. Entonces,
necesitamos modificar la url de dicho cliente para que, desde el interior del contenedor pueda comunicarse al
exterior, hacia nuestro `course-service`.

Recordemos que la url del `course-service` lo definimos en el `application.yml` del `user-service`, así que eso es
lo único que modificaremos. En vez de `localhost` lo cambiaremos por el `host.docker.internal`.
...
El microservicio `course-service` también hacemos uso del `RestClient` para comunicarnos con el `user-service`. En
este caso, como la comunicación es desde la máquina local hacia dentro del contenedor, no es necesario cambiar la url
definido en el `RestClient`, dado que el puerto externo que le definimos al contenedor del `user-service` es el mismo
que está usando el `RestClient` (`8001`).

Recordemos que esa configuración también lo tenemos en el `application.yml` del `course-service`.
...
Como hemos modificado el código fuente del `user-service` es necesario volver a generar la imagen de la aplicación y
luego crear y ejecutar nuevamente un contenedor.

Así que iniciamos generando previamente el `jar` con los cambios aplicados.
...
$ docker image ls
REPOSITORY     TAG       IMAGE ID       CREATED          SIZE
user-service   latest    7e42c7fee290   34 seconds ago   673MB
<none>         <none>    546c1ada1f69   20 minutes ago   673MB
````
...
9f79fa485ae8adfe6e961ea686e857e4133e55356bf6322e185d12f44b0baac3

$ docker container ls -a
CONTAINER ID   IMAGE          COMMAND                  CREATED          STATUS          PORTS                    NAMES
9f79fa485ae8   user-service   "/__cacert_entrypoin…"   14 seconds ago   Up 12 seconds   0.0.0.0:8001->8001/tcp   agitated_agnesi
````

Una vez que ya tenemos corriendo nuestro contenedor del microservicio `user-service`, levantamos nuestro
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 21 May 2025 04:46:53 GMT
<
{
  "id": 3,
  "name": "Kubernetes",
...
>
< HTTP/1.1 204
< Date: Wed, 21 May 2025 04:48:57 GMT
<
````

Si vemos el curso con id 3, veremos de que el usuario con id 5 ya no está porque fue eliminado.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 21 May 2025 04:49:30 GMT
<
{
  "id": 3,
  "name": "Kubernetes",
