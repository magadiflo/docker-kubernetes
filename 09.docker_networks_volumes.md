...
`PostgreSQL`. Estos contenedores nos permitirán habilitar la línea de comando para interactuar y realizar consultas a
...
En este punto del curso tenemos 4 contenedores ejecutándose de los cuales nos vamos a enfocar en trabajar con los
contenedores de bases de datos de `mysql` y `postgres`.

````bash
$ docker container ls -a
CONTAINER ID   IMAGE                 COMMAND                  CREATED          STATUS          PORTS                               NAMES
47afe8c9bba9   mysql:8.0.41-debian   "docker-entrypoint.s…"   3 minutes ago    Up 3 minutes    33060/tcp, 0.0.0.0:3307->3306/tcp   c-mysql
8348796416ea   postgres:17-alpine    "docker-entrypoint.s…"   3 minutes ago    Up 3 minutes    0.0.0.0:5433->5432/tcp              c-postgres
dee698e03458   course-service        "/__cacert_entrypoin…"   12 minutes ago   Up 12 minutes   0.0.0.0:8002->8002/tcp              c-course-service
bb939039cd25   user-service          "/__cacert_entrypoin…"   12 minutes ago   Up 12 minutes   0.0.0.0:8001->8001/tcp              c-user-service
````
...
poder interactuar con la base de datos de `MySQL` que actualmente ya lo tenemos ejecutando en el contenedor `c-mysql`.

````bash
$ docker container run -it --rm --network docker-kubernetes-net mysql:8.0.41-debian /bin/sh
# mysql -hc-mysql -uadmin -p
Enter password:
Welcome to the MySQL monitor.  Commands end with ; or \g.
Your MySQL connection id is 18
Server version: 8.0.41 MySQL Community Server - GPL
...
- `--network docker-kubernetes-net`, debe ser la misma red en la que se encuentra el contenedor `c-mysql`.
- `mysql:8.0.41-debian`, imagen a partir de la cual crearemos un contenedor al vuelo, un utilitario.
- `/bin/sh` o `/bin/bash`, línea de comando que usaremos dentro del contenedor.
...
- `mysql -hc-mysql -uadmin -p`, donde:
    - `mysql`, este es el comando del cliente de `MySQL`. Es la herramienta utilizada para interactuar con el servidor
      `MySQL` desde la línea de comando permitiéndote ejecutar consultas y gestionar la base de datos.
...
La siguiente instrucción nos permite crear un `contenedor utilitario` que nos `proporciona una línea de comandos` para
poder interactuar con la base de datos de `PostgreSQL` que actualmente se está ejecutando en el contendor `c-postgres`.

````bash
$ docker container run -it --rm --network docker-kubernetes-net postgres:17-alpine /bin/sh
...
-------------------+----------+----------+-----------------+------------+------------+--------+-----------+-----------------------
 db_course_service | postgres | UTF8     | libc            | en_US.utf8 | en_US.utf8 |        |           |
...
- `-it`, habilitar el terminal interactivo.
- `--rm`, cuando se detenga este contenedor, se eliminará automáticamente.
- `--network docker-kubernetes-net`, debe ser la misma red en la que se encuentra el contenedor `c-postgres`.
- `postgres:17-alpine`, imagen a partir de la cual crearemos un contenedor al vuelo, un utilitario.
- `/bin/sh` o `/bin/bash`, línea de comando que usaremos dentro del contenedor.
...
- `psql -hc-postgres -Upostgres`, donde:
    ...
    - `-Upostgres`, el usuario (`-U`) es el que definimos al crear el contenedor de postgres, en nuestro caso es
      `postgres`.
    - Luego, cuando demos enter, en la siguiente línea pedirá ingresar el password que es `magadiflo`.
