...
Actualmente, los servicios `user-service` y `course-service` están usando los bloques `build` para construir sus
imágenes a partir de un `Dockerfile`. Veamos un extracto del servicio `user-service` para recordar cómo es que estamos
...
Realizamos las modificaciones a los servicios `user-service` y `course-service` del archivo `compose.yml` para que
traiga las imágenes de `Docker Hub`.
...
Entonces, como ahora traerémos las imágenes de `Docker Hub` ya no necesitamos usar el bloque `build` por eso lo eliminé.
...
[+] Running 17/17                     
 ✔ user-service Pulled                
   ...
   ✔ 14583e96d3e1 Already exists      
   ✔ d0963fa73eb4 Already exists      
 ✔ course-service Pulled              
   ✔ b66e5378b320 Already exists      
   ✔ f806a702b9fa Already exists      
   ✔ 7b81a8fb2a4a Already exists      
   ✔ 896e4129cbb1 Already exists      
[+] Running 4/4                       
 ✔ Container c-postgres        Healthy
 ✔ Container c-mysql           Healthy
 ✔ Container c-user-service    Started
