...
````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"name\": \"Roxana\", \"email\": \"roxana@gmail.com\", \"password\": \"123456\"}" http://localhost:8001/api/v1/users | jq
>
< HTTP/1.1 201
...
````bash
$ curl -v -X PUT -H "Content-Type: application/json" -d "{\"name\": \"Liz\", \"email\": \"liz@gmail.com\", \"password\": \"123456\"}" http://localhost:8001/api/v1/users/4 | jq
>
< HTTP/1.1 200
< Content-Type: application/json
<
...
- Valida el registro de un usuario cuando no se le envían datos

````bash
$ curl -v -X PUT -H "Content-Type: application/json" -d "{}" http://localhost:8001/api/v1/users/4 | jq
>
< HTTP/1.1 400
<
{
  ...
  "message": "Falló la validación de los campos",
  "timestamp": "2024-09-25T23:27:13.5094537",
  "path": "/api/v1/users/4",
  "errors": {
    "password": [
      "must not be blank"
...
````bash
$ curl -v -X PUT -H "Content-Type: application/json" -d "{\"name\": \"Liz\", \"email\": \"martin@gmail.com\", \"password\": \"123456\"}" http://localhost:8001/api/v1/users/3 | jq
>
< HTTP/1.1 400
<
{
  ...
  "timestamp": "2024-09-25T23:28:55.6076843",
  "path": "/api/v1/users/3"
}
````
...
<
{
  ...
  "timestamp": "2024-09-25T23:29:50.7022068",
